...
public class ValueElementInfo extends AbstractElementInfo implements ElementInfo {
    ...
    String usage;
    /**
     * 如果为类集，则为List或者Set的子类
     */
    Class collectionClass;

    private ValueElementInfo(InterfaceInfo interfaceInfo, ElementSet elementSet, String fullName) {
        super(interfaceInfo, elementSet, fullName);
    }


    public static ValueElementInfoBuilder builder(InterfaceInfo interfaceInfo, ElementSet elementSet, String fullName) {
        return new ValueElementInfoBuilder(interfaceInfo, elementSet, fullName);
    }
     ...
     * @return 枚举描述
     */
    public String getEnumDesc(Object enumCode) {
        return enums.get(StringUtils.safeToString(enumCode, "").trim());
    }
     ...
     * @return 枚举代码
     */
    public String getEnumCode(Object enumDesc) {
        String enumCode = null;
        for (Map.Entry<String, String> entry : enums.entrySet()) {
            if (entry.getValue().equals(StringUtils.safeToString(enumDesc, "").trim())) {
                enumCode = entry.getKey();
                break;
            }
        }
        return enumCode;
    ...
    public static class ValueElementInfoBuilder {
        ...
        private ValueElementInfoBuilder(InterfaceInfo interfaceInfo, ElementSet elementSet, String fullName) {
        ...
        public ValueElementInfoBuilder collectionClass(Class collectionClass) {
        ...
        public ValueElementInfo build() {
            ValueElementInfo value = new ValueElementInfo(interfaceInfo, elementSet, fullName);
            if (desc == null) {
                value.desc = fullName;
            } else {
                value.desc = desc;
            ...
            value.unique = unique;
            value.usage = usage == null ? "" : usage;
            value.javaClass = javaClass;
            value.enums.putAll(enums);
            value.collectionClass = collectionClass == null ? Object.class : collectionClass;
            value.required = required;
            value.minLen = minLen;
            value.maxLen = maxLen;
            value.defaults.addAll(defaults);
            value.patterns.addAll(patterns);
            value.patternType = patternType;
    ...
    @Override
    public int hashCode() {
        return Objects.hash(interfaceInfo, columnType, getFullName());
    }

    @Override
    public String toString() {
        return interfaceInfo.getFullName() + ":" + getFullName();
    }
}
